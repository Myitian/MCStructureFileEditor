...
namespace fNbt {
    /// <summary> 一个高效的写入器，用于将NBT数据直接写入流中。
    /// NbtWriter的每个实例都会写入一个完整的文件。 
    /// NbtWriter执行NBT文件格式的所有约束，
    /// 除了检查一个复合体中是否有重复的标签名称。 </summary>
    public sealed class NbtWriter {
        ...
        /// <summary> 初始化一个新的NbtWriter类的实例。 </summary>
        /// <param name="stream"> 目标流。 </param>
        /// <param name="rootTagName"> 根标签名字（立即写入）。 </param>
        /// <remarks> 假设流中的数据应该是Big-Endian编码的。</remarks>
        /// <exception cref="ArgumentNullException"> <paramref name="stream"/> 或 <paramref name="rootTagName"/> 为 <c>null</c>. </exception>
        /// <exception cref="ArgumentException"> <paramref name="stream"/> 是不可写入的。 </exception>
        ...
        /// <param name="stream"> 目标流。 </param>
        /// <param name="rootTagName"> 根标签名字（立即写入）。 </param>
        /// <param name="bigEndian"> NBT的数据是否应该采用Big-Endian编码。 </param>
        /// <exception cref="ArgumentNullException"> <paramref name="stream"/> 或 <paramref name="rootTagName"/> 为 <c>null</c>. </exception>
        /// <exception cref="ArgumentException"> <paramref name="stream"/> 是不可写入的。 </exception>
